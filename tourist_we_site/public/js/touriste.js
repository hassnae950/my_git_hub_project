...
const Tourist_table = document.querySelector('#Tourist_table');


const toPDF = function (Tourist_table) {
    const html_code = `
    <!DOCTYPE html>
    <link rel="stylesheet" type="text/css" href="css/touriste.css">
    <main class="table" id="Tourist_table">
    ${Tourist_table.innerHTML}</main>`;
...
const toExcel = function (table) {
    ...
    const wb = XLSX.utils.table_to_book(tempTable, { sheet: "Touristes" });
    ...
    a.download = "touristes_list.xlsx"; // Nom du fichier Excel
...
excel_btn.onclick = () => {
    const table = document.querySelector('#Tourist_table'); // Sélectionner le tableau avec l'ID "Tourist_table"
    if (table) {
