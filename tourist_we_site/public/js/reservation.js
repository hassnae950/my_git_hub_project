...
const rese_table = document.querySelector('#rese_table');


const toPDF = function (rese_table) {
    const html_code = `
    <!DOCTYPE html>
    <link rel="stylesheet" type="text/css" href="css/touriste.css">
    <main class="table" id="rese_table">
    ${rese_table.innerHTML}</main>`;
...
const toExcel = function (table) {
    ...
    const wb = XLSX.utils.table_to_book(tempTable, { sheet: "Reservations" });
    ...
    a.download = "Reservations_table.xlsx"; // Nom du fichier Excel
...
excel_btn.onclick = () => {
    const table = document.querySelector('#rese_table'); // Sélectionner le tableau avec l'ID "rese_table"
    if (table) {
